...
//                $hash=password_hash("rasmuslerdorf", PASSWORD_DEFAULT);
    ...
    function insertEvent( $eventid, $name, $description, $location, $status, $date, $eventcreator, $starttime, $endtime)
    {
        ...
        if ($stmt = $mysqli->prepare("INSERT INTO event values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"))
        {
            ...
            $stmt -> bind_param("isssissssi", $eventid, $name, $description, $location, $status, $date, $eventcreator, $starttime, $endtime, $isdeleted);
            if ($stmt->execute())
            {
                $stmt->close();
                $mysqli->close();
...
function getNewStaffId()
{
    ...
    if ($stmt = $mysqli->prepare("SELECT MAX(StaffId)+1 FROM staff LIMIT 1;"))
    {
        if ($stmt->execute())
        {
